...
import { getElevatedUsers, changeUserRole } from "@/src/features/admin/api/adminApi";
...
export default function ElevatedUsersList() {
  ...
  const { loading, error, response: users, reload } = useLoading(getElevatedUsers);

  const allUsers = users ?? [];
  const pageCount = Math.ceil(allUsers.length / size);
  const visible = allUsers.slice(page * size, page * size + size);

  async function handleRoleChange(user: ElevatedUserDTO, direction: "up" | "down") {
    const idx = SITE_ROLES.indexOf(user.role);
    const newRole = direction === "up" ? SITE_ROLES[idx + 1] : SITE_ROLES[idx - 1];
    if (!newRole) return;
      ...
      <div className="adminPaginationBar">
        ...
        <div className="adminPageControls">
          <button
            ...
            disabled={page === 0}
          >←</button>
          <span className="itemMeta">{page + 1} / {Math.max(1, pageCount)}</span>
          <button
            className="actionButton"
